...
//			});
			...
			final String msg=bundle.getString(JPushInterface.EXTRA_MESSAGE);
			for (int i=0;i<PushTagOrAliasList.size();i++){
				LogUtils.e("PushTagOrAliasList--->"+PushTagOrAliasList.get(i).toString());
			}
			...
			LogUtils.e("PushReceiver收到推送消息--->"+jsonObjectElement.toString());
			//用于过滤不属于当前登录人的信息
...
//			}
			...
			showInspectorRecordNotification(context, LocaleUtils.getI18nValue(message));
		}catch (Throwable throwable){
			LogUtils.e("出异常--->"+throwable.getMessage());
			throwable.printStackTrace();
		}
	}
	private void showInspectorRecordNotification(Context context,String message) {
		LogUtils.e("开始执行消息通知框--->"+message);
