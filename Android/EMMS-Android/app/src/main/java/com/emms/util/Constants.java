package com.emms.util;

/**
 * Created by jaffer.deng on 2016/6/6.
 */
public class Constants {
    public static final int REQUEST_CODE_TASK_DETAIL_TO_NEW_CAPTURE_ACTIVITY = 1;
    public static final int REQUEST_CODE_FROZEN_ACCOUNT = 110;
    public static final int REQUEST_CODE_IDENTITY_AUTHENTICATION_FAIL = 111;
    public static final int REQUEST_CODE_IDENTITY_AUTHENTICATION_SUCCESS = 112;
    public static final int REQUEST_CODE_IDENTITY_AUTHENTICATION_SUCCESS_AUTO = 113;

    public static final int REQUEST_CODE_EXIT_LOGIN_SUCCESS = 114;
    public static final int REQUEST_CODE_EXIT_LOGIN_FAIL = 115;

    public final static int REQUEST_CODE_EXCHANGE_ORDER=116;
    public final static int REQUEST_CODE_PROCESSING_ORDER_TASK_DETAIL=117;

    public final static int REQUEST_CODE_TASKHISTORY=118;

    public final static int REQUEST_CODE_MEASURE_POINT=119;
    public final static int REQUEST_CODE_END_TASK=120;
    public final static int REQUEST_CODE_END_TASK_TO_INVITOR_ACTIVITY=121;

    public final static int REQUEST_CODE_TASK_DETAIL_TO_CAPTURE_ACTIVITY=122;
    public final static int RESULT_CODE_CAPTURE_ACTIVITY_TO_TASK_DETAIL=123;
    public final static int REQUEST_CODE_STYLE_CHANGE_PROCESSING_ORDER_TASK_DETAIL=124;
    public final static int REFRESH_PARENT_PAGE_DATA = 125;

    public final static String FLAG_CREATE_CAR_MOVING_TASK="CreateCarMovingTask";
    public final static String FLAG_CREATE_SHUNTING_TASK="CreateShuntingTask";
    public final static String FLAG_FAULT_NOT_SOLVE = "faultNotSolve";
}
