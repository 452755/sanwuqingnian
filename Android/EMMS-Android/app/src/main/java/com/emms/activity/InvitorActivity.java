...
public class InvitorActivity extends NfcActivity implements View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(isExChangeOrder){
            ((TextView)findViewById(R.id.tv_title)).setText(LocaleUtils.getI18nValue("exchangeOrder"));
        } else {
            ((TextView) findViewById(R.id.tv_title)).setText(LocaleUtils.getI18nValue("invitor_help"));
        }
        ...
        if(Tag!=null){
            ((TextView)findViewById(R.id.tv_title)).setText(LocaleUtils.getI18nValue("AddTaskPeople"));
                    ...
                    public void run() {
                        if(adapter.getListItems()!=null){
                            if(Tag!=null){
                                ...
                                for(int j=0;j<TaskParticipantsList.size();j++){
                                  ...
                                  if(   DataUtil.isDataElementNull(adapter.getListItems().get(adapter.getListItemID().get(k)).get(Operator.OPERATOR_ID))
                                      ...
                                      ToastUtil.showToastShort(DataUtil.isDataElementNull(TaskParticipantsList.get(j).asObjectElement().get("Name"))
                                              +" "+LocaleUtils.getI18nValue("JoinerIsInTask"),context);
                                      return;
                                  }
                                    }
                                }
                                ...
                                if(data.size()<=0){
                                    ToastUtil.showToastShort(LocaleUtils.getI18nValue("pleaseSelectJoiner"),context);
                                    return;
                                }
                        ...
                        for(int i=0;i<adapter.getlistItemID().size();i++){
                            ...
                            if(invitorList.size()==0){
                                if(isExChangeOrder){
                                    ToastUtil.showToastShort(LocaleUtils.getI18nValue("pleaseSelectExchangeOrderTarget"),context);
                                    return;
                                }else {
                                    ToastUtil.showToastShort(LocaleUtils.getI18nValue("pleaseSelectInviteTarget"),context);
                                    return;
                                }
                            }
                            for(int j=0;j<TaskOperator.size();j++) {
                                if (invitorList.contains(Integer.valueOf(TaskOperator.get(j)))) {
                                    if(isExChangeOrder){
                                        ToastUtil.showToastShort(LocaleUtils.getI18nValue("exchangerIsInTask"),context);
                                    }else{
                                        ToastUtil.showToastShort(LocaleUtils.getI18nValue("invitorIsInTask"),context);
                                    }
                                    return;
                                }
                            }
    ...
    public void postInviteDataToServer(ArrayList<Integer> submitData){
        if(isExChangeOrder){
            if(submitData.size()>1){
                Toast toast=Toast.makeText(this,LocaleUtils.getI18nValue("exchangeOrderToOnePerson"),Toast.LENGTH_LONG);
                toast.setGravity(Gravity.CENTER,0,0);
                toast.show();
                return;
            }
                ...
                if(t!=null){
                 ...
                 if(Tag==null) {
                     if (isExChangeOrder) {
                         ...
                         Toast.makeText(InvitorActivity.this, LocaleUtils.getI18nValue("exchangeOrderSuccess"), Toast.LENGTH_LONG).show();
                     } else {
                         Toast.makeText(InvitorActivity.this, LocaleUtils.getI18nValue("inviteSuccess"), Toast.LENGTH_LONG).show();
                     }
                 }else {
                     Toast.makeText(InvitorActivity.this, LocaleUtils.getI18nValue("SuccessAddTaskPeople"), Toast.LENGTH_LONG).show();
                     //setResult(3);
                 }
                finish();
            }else {
                        if(Tag==null) {
                        if(isExChangeOrder){
                            ToastUtil.showToastShort(LocaleUtils.getI18nValue("exchangeOrderFail"),context);
                        }else {
                            ToastUtil.showToastShort(LocaleUtils.getI18nValue("inviteFail"),context);
                        }
                        }else {
                            ToastUtil.showToastShort(LocaleUtils.getI18nValue("FailAddTaskPeople"),context);
                        }
                    }
                }
            }
            ...
            public void onSuccess(String t) {
               ...
               if(t!=null){
                 ...
                 for(int i=0;i<json.get(Data.PAGE_DATA).asArrayElement().size();i++){
                       ...
                       ToastUtil.showToastShort(LocaleUtils.getI18nValue("getTaskOperatorFail"),context);
                   }
               }else{
                   ToastUtil.showToastShort(LocaleUtils.getI18nValue("getTaskOperatorFail"),context);
               }
