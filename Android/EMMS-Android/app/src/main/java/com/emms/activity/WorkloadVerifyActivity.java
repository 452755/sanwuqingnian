...
public class WorkloadVerifyActivity extends NfcActivity  implements OnTabSelectListener,View.OnClickListener{
    ...
    private String[] mTitles = { LocaleUtils.getI18nValue("select_tab_verify_status_unaudited"), LocaleUtils.getI18nValue("select_tab_verify_status_audited"), LocaleUtils.getI18nValue("select_tab_verify_status_query")};
    ...
    private void initView() {
        ((TextView) findViewById(R.id.tv_title)).setText(LocaleUtils.getI18nValue("workloadVerify"));
        findViewById(R.id.btn_right_action).setOnClickListener(this);
        ((Button)findViewById(R.id.btn_sure_bg)).setText(LocaleUtils.getI18nValue("warning_message_confirm"));
...
//    }
        ...
        switch (id) {
            ...
            case R.id.search_button:{
                if(filterCondition.getText().equals("")){
                    ToastUtil.showToastShort(LocaleUtils.getI18nValue("pleaseSelectCondition"),mContext);
                    return;
                }
                int filterTimeLong=30;
                if(filter_condition_map.get(filterCondition.getText())==4){
                    if(filterTime.getText().toString().equals("")){
                        ToastUtil.showToastShort(LocaleUtils.getI18nValue("pleaseInputFilterTime"),mContext);
                        return;
                    }
                    if( !DataUtil.isNum(filterTime.getText().toString())
                        ...
                        ToastUtil.showToastShort(LocaleUtils.getI18nValue("pleaseInputIntegerLargeThanZero"),mContext);
                        return;
                    }
    ...
    private void  doSubmit(){
        if(vp.getCurrentItem()==0){
            ...
            ToastUtil.showToastShort(LocaleUtils.getI18nValue("NoDataToSubmit"),mContext);
        }
    }
    private void initSearchView() {
        ((TextView) findViewById(R.id.tvFilterCondition)).setText(LocaleUtils.getI18nValue("filterCondition"));
        ((Button) findViewById(R.id.search_button)).setText(LocaleUtils.getI18nValue("sure"));
        filterCondition.setHint(LocaleUtils.getI18nValue("select"));
        filterTime.setHint(LocaleUtils.getI18nValue("pleaseInputFilterTime"));
        ...
        initDropSearchView(null, filterCondition.getmEditText(), LocaleUtils.getI18nValue("filterConditionTitle"),DataDictionary.DATA_NAME,
                1, LocaleUtils.getI18nValue("getDataFail"),filterCondition.getDropImage());
    ...
    private void initData(){
        ...
        data1.set(DataDictionary.DATA_NAME, LocaleUtils.getI18nValue("filterCondition1"));
        JsonObjectElement data2=new JsonObjectElement();
        data2.set(DataDictionary.DATA_NAME, LocaleUtils.getI18nValue("filterCondition2"));
        JsonObjectElement data3=new JsonObjectElement();
        data3.set(DataDictionary.DATA_NAME, LocaleUtils.getI18nValue("filterCondition3"));
        filterConditionList.add(data1);
        filterConditionList.add(data2);
        filterConditionList.add(data3);

        filter_condition_map.put( LocaleUtils.getI18nValue("filterCondition1"),2);
        filter_condition_map.put( LocaleUtils.getI18nValue("filterCondition2"),3);
        filter_condition_map.put( LocaleUtils.getI18nValue("filterCondition3"),4);
    }
