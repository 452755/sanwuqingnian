    ...
    public void start(JSONObject options, UniJSCallback callback) throws JSONException {
        try {
            ...
            if(mUniSDKInstance != null && mUniSDKInstance.getContext() instanceof Activity) {
                Activity mainActivity = ((Activity) mUniSDKInstance.getContext());
                ...
                intent.putExtra("mainActivityClassName", mainActivity.getLocalClassName());
...
//                 添加任务
    ...
    @UniJSMethod(uiThread = true)
    public void resetContent(JSONObject options, UniJSCallback callback) throws JSONException {
        try {
            Log.e(TAG, "resetContent--"+options);
            if(mUniSDKInstance != null && mUniSDKInstance.getContext() instanceof Activity) {
                ForegroundService.resetNotifyContent(options.getString("title"), options.getString("text"));
            }
            if(callback != null) {
                JSONObject data = new JSONObject();
                data.put("code", 1);
