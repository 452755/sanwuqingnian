...
namespace GerberVS
{
    ...
    public class GerberLineReader
    {
        ...
        /// <summary>
        /// Read the next character in the line or start on the next line if at the end of the current line.读取行中的下一个字符，如果在当前行的末尾，则从下一行开始。
        /// </summary>
        ...
        public char Read()
        {
            ...
            if (Position >= LineLength)      // At the end of the line, read the next one.在这行的最后，读下一个。
            {
                CurrentLine = streamReader.ReadLine();
                LineNumber++;
                if (String.IsNullOrEmpty(CurrentLine))  // Empty line, return new line character.空行，返回新行字符。
                    return '\n';
        ...
        /// <summary>
        /// Reads in digits from the current file position and converts them to an integer.从当前文件位置读取数字并将其转换为整数。
        /// </summary>
