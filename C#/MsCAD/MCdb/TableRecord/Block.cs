...
namespace MsCAD.DatabaseServices
{
    public class Block : DBTableRecord, IEnumerable<Entity>
    {
        ...
        public override void XmlOut(Filer.XmlFiler filer)
        {
            ...
            foreach (Entity item in _items)
            {
                filerImpl.NewSubNodeAndInsert(item.GetType().Name);//item.className FullName
                item.XmlOut(filer);
                filerImpl.Pop();
            }
            filerImpl.Pop();
        ...
        public override void XmlIn(Filer.XmlFiler filer)
        {
            ...
            if (entitiesNode != null && entitiesNode.ChildNodes != null)
            {
                ...
                {
                    //Type type = Type.GetType("MsCAD.DatabaseServices." + entityNode.Name);
                    if (!entityNode.Name.Contains(ns))
                        name = ns + entityNode.Name;
                    else
